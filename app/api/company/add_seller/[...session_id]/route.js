...
export async function POST(req, { params }) {
    try {
        ...
        const result = await db.query('INSERT INTO seller (s_id,s_type, s_name, s_mail,s_pwd) VALUES (?,?, ?, ?,?)', [s_id,s_type, s_name, s_mail,s_pwd]);
        const result1 = await db.query("insert into company_seller(c_id,s_id) values(?,?)",[c_id[0].c_id,s_id]);
        db.release();
